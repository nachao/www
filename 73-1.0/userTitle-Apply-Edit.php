...
?>
				...
				<div class="leftarea f">
					...
					<div class="commarea applyTitle">
						<div class="content">
							...
							<form class="formlist" method="get" onsubmit="return detection(false);" >
								<?php
								//判断是否有参数 并 判断是否有指定标题
								...
								<div class="row">
									<div class="radio">
										...
										<label><input type="radio" name="types" value="3" />小组</label>
	...
	<script type="text/javascript">
		...
		function detection(sub){
			...
			//获取参数
			var value = depict.val().replace(/\s/g,''),
				length = value.length;
			...
			//检查是否有输入标题
			if ( title.val().replace(/\s/g,'').length < 5 ) { 
				...
				doTitle(function(is){
					if ( !is ) {
						...
						if( length < 10 || length > 500 ){
							...
							//检测余额是否足够
							if ( $.Usum() <= parseInt($('#totalTote').attr('tote')) ){
								$('#tipApply').show();
							}else{
