<?php
	...
	if(isset($_POST['submit'])){
		if(isset($_POST['reviseCon'])){		//修改内容
			$cid = $_POST['reviseCon'];
			$Rinfo = $c -> Ginfo($cid);
			if($_POST['types'] == 1){		//图片类型
				if($_POST['beforeimg'] != $Rinfo['image']){
					$c -> Uimg($cid, $_POST['beforeimg']);
				}
				if($_POST['imgDepict'] != $Rinfo['content']){
					$c -> Utxt($cid, $_POST['imgDepict']);
				}
...
?>
								...
								<form class="publish" enctype="multipart/form-data" action="" method="post" name="upform" onsubmit="return check(this);">
									...
									<div class="main">
										...
										<div class="col extent picture">
											...
											<div class="c"></div>
											<!-- <div class="are">
												<textarea id="imgDepict" class="cue" name="imgDepict" placeholder="描述（可以不写）" ></textarea>
											</div> -->
    										<script type="text/plain" id="j-depict-image" style="width:706px;height:150px;"></script>
	...
	<script type="text/javascript">
        ...
        //实例化编辑器
        // UM.getEditor('j-writing');
        // UM.getEditor('j-depict-image', {
        // 	toolbar: '';
        // });
